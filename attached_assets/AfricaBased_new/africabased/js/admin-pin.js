(function () {
  ...
  function getAdminKey() {
    return window.__abAdminKey || '';
  }

  window.__abAdminReady = new Promise(function(resolve) {
    window.__abAdminResolve = resolve;
  });

  function fetchAdminKey(cb) {
    fetch('/api/admin/me').then(function(r) { return r.json(); }).then(function(data) {
      if (data && data.admin) {
        window.__abAdminKey = data.admin.api_key || (data.isSuper ? '1540568e' : '');
        window.__abAdminData = data.admin;
        window.__abIsSuper = !!data.isSuper;
        window.__abIsSubAdmin = !!data.admin.isSubAdmin;
      }
      if (window.__abAdminResolve) { window.__abAdminResolve(); window.__abAdminResolve = null; }
      cb();
    }).catch(function() {
      if (window.__abAdminResolve) { window.__abAdminResolve(); window.__abAdminResolve = null; }
      cb();
