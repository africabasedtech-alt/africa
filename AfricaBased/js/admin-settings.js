  ...
  el.innerHTML = opportunities.map(o => `
    <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
      ...
      <div style="flex:1;min-width:160px;">
        ...
        <p style="color:rgba(255,255,255,0.5);font-size:0.85rem;margin:2px 0 0;">${o.category} &nbsp;|&nbsp; KSH ${Number(o.price||0).toLocaleString()}</p>
      </div>
      <div style="display:flex;gap:8px;">
...
window.startEditOpportunity = function(id) {
  ...
  document.getElementById('opportunityPrice').value = o.price;
...
function setupOpportunities() {
  ...
  document.getElementById('saveOpportunityBtn').onclick = async () => {
    ...
    if (!name || !description || isNaN(price)) { toast('Name, description and price are required.', false); return; }
    const body = { name, image, description, category, price, featured };
    try {
