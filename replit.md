...
- **Payment Channels System:** Admin-managed payment channels (M-Pesa numbers, Till numbers, Paybill numbers, Bank transfers, Other) stored in `payment_channels` table. Admin CRUD at `/api/admin/payment-channels`. When deposit mode is manual, `/api/deposit/config` randomly selects an active channel and auto-generates step-by-step instructions based on channel type. Admin page at `admin-payment-channels.html`, linked from Admin Panel.
